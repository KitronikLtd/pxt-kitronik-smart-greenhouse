...
namespace kitronik_smart_greenhouse {
	...
	*/
     ...
     * @param powerOP is an optional parameter to set the power output for the pin eg: 100
     */
    //% subcategory="Inputs/Outputs"
    ...
    //% weight=80 blockGap=8
    export function controlHighPowerPin(pin: kitronik_smart_greenhouse.HighPowerPins, output: boolean, powerOP: number = 100): void {
        if (pin == 13) {
            if (output == true) {
                if (powerOP != 100) {
                    let pin13Analog = powerOP * (1023/100)
            ...
            if (output == true) {
                if (powerOP != 100) {
                    let pin14Analog = powerOP * (1023/100)
